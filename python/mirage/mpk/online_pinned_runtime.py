...
class OnlinePinnedRuntime:
    ...
    def submit(
        ...
        with self._ring_lock:
            ...
            self._publish_request_locked(
                slot, rid, token_ids, initial_step, sampling_lanes)
    ...
    def flush_waiting(self) -> int:
        ...
        """
        # Pop and publish atomically under both locks (ring -> waiting, same
        # order as submit()).  The GPU consumes the ring strictly in order,
        # so an unpublished reserved slot would permanently stall every
        # request behind it — hence the appendleft rollback on failure.
        with self._ring_lock:
            slot = self._cpu_req_tail & self._mask
            if self._load_i32_acquire(self._req_ready, slot) != 0:
            ...
            rid, token_ids, initial_step, sampling_lanes = request
            try:
                self._publish_request_locked(
                    slot, rid, token_ids, initial_step, sampling_lanes)
            except Exception:
    ...
    def _publish_request_locked(
        ...
        Sampling lanes are written BEFORE the release-store of ready=1 so the
        GPU's acquire-load of ready observes a complete record (same ordering
        contract as prompt_len / the inbox tokens)."""
